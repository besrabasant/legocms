...
if (!function_exists('legocmsLocale')) {
    /**
     * LegoCMS Locale
     *
     * @return  void
     */
    function legocmsLocale()
    {
        return \app()->getLocale();
    }
}

if (!function_exists('legocmsTrans')) {
    /**
     * LegoCMS Translation helper
     *
     * @param  string  $key
     * @param  array  $replace
     *
     * @return  void
     */
    function legocmsTrans($key, $replace = [])
    {
        $locale = \config('legocms.locale', \config('legocms.fallback_locale', 'en'));
...
if (!function_exists('getLocales')) {
    /**
     * Return all locales enabled in LegoCMS.
     *
     * @return  string[]
     */
    function getLocales()
...
if (!function_exists('getLanguagesForVueStore')) {
    /**
     * Return languages config for VUe Form Store.
     * 
    ...
    function getLanguagesForVueStore($form_fields = [], $translate = true)
    {
        ...
        if ($manageMultipleLanguages && $translate) {
            $allLanguages = Collection::make(\config('translatable.locales'))->map(
                function ($locale, $index) use ($form_fields) {

                    $field = Arr::first($form_fields['translations'], function ($value) use ($locale) {
                    ...
                    return [
                        ...
                        'value' => $locale,
                        'disabled' => false,
                        'published' => $field['active'] ?? ($index === 0),
                    ];
                }
            );
            ...
            return [
                'all' => $allLanguages,
                'active' => \request()->has('lang') ?
                    $allLanguages->firstWhere('value', \request('lang')) :
                    null
            ];
        }
