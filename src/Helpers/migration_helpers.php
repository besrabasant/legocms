...
if (!function_exists('legocmsIncrementsMethod')) {
    /**
     * @return  string
     */
    function legocmsIncrementsMethod()
    {
        return \config('legocms.migrations_use_big_integers')
            ? 'bigIncrements'
            : 'increments';
    }
}

if (!function_exists('legocmsIntegerMethod')) {
    /**
     * @return  string
     */
    function legocmsIntegerMethod()
    {
        return \config('legocms.migrations_use_big_integers')
            ? 'bigInteger'
            : 'integer';
...
if (!function_exists("createDefaultTableFields")) {
    ...
    {
        $table->{\legocmsIncrementsMethod()}('id');
...
if (!function_exists("createDefaultTranslationsTableFields")) {
    ...
    function createDefaultTranslationsTableFields($table, $tableNameSingular, $tableNamePlural = null)
    {
        ...
        $table->{\legocmsIncrementsMethod()}('id');
        $table->{\legocmsIntegerMethod()}("{$tableNameSingular}_id")->unsigned();
        ...
        $table->foreign(
            ...
            "fk_{$tableNameSingular}_translations_{$tableNameSingular}_id"
        )
            ->references('id')
            ->on($tableNamePlural)
            ->onDelete('CASCADE');
...
if (!function_exists('createDefaultRelationshipTableFields')) {
    ...
    function createDefaultRelationshipTableFields($table, $table1NameSingular, $table2NameSingular, $table1NamePlural = null, $table2NamePlural = null)
    {
        ...
        $table->{legocmsIntegerMethod()}("{$table1NameSingular}_id")->unsigned();
        $table->{legocmsIntegerMethod()}("{$table2NameSingular}_id")->unsigned();
        ...
        $table->index(
            ["{$table2NameSingular}_id", "{$table1NameSingular}_id"],
            "idx_{$table1NameSingular}_{$table2NameSingular}_" . Str::random(5)
        );
    }
}
...
if (!function_exists('createDefaultRevisionsTableFields')) {
    ...
    function createDefaultRevisionsTableFields($table, $tableNameSingular, $tableNamePlural = null)
    {
        ...
        $table->{\legocmsIncrementsMethod()}('id');
        $table->{\legocmsIntegerMethod()}("{$tableNameSingular}_id")->unsigned();
        $table->{\legocmsIntegerMethod()}('user_id')->unsigned()->nullable();
        ...
        $table->foreign('user_id')->references('id')->on(
            \config('legocms.users_table', 'legocms_users')
        )->onDelete('set null');
    }
}
